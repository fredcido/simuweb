...
Fefop.EDCContract = {
	...
	submit = function()
	{
		...
		{
		    $( '#edc-contract-list tbody' ).empty();
	     
		    oTable = $( '#edc-contract-list' ).dataTable();
		    oTable.fnDestroy(); 

		    $( '#edc-contract-list tbody' ).html( response );
		    
		    General.drawTables( '#edc-contract-list' );
		    General.scrollTo( '#edc-contract-list', 800 );
		},
		error: function ()
		{
	...
	General.execFunction( Fefop.EDCContract[method], pane );
    },
	...
	submit = function()
	{
		...
		callback: function( response )
		{
			...
			if ( General.empty( $( '#id_edc_contract' ).val() ) ) {
			    
			    window.setTimeout(
				function()
				{
				    General.go( "/fefop/edc-contract/edit/id/" + response.id );
				},
				3000
			    );
			}
	...
	var calcDate = function()
	{
	    ...
	    $.ajax(
		{
		    type: 'POST',
		    dataType: 'json',
		    url: General.getUrl( '/fefop/edc-contract/calc-diff-days/' ),
		    data: {
	...
	$( '#fk_id_adddistrict' ).change(
		...
		url = '/fefop/edc-contract/search-sub-district/id/' + $( this ).val();
	...
	$( '#fk_id_addsubdistrict' ).change(
		...
		url = '/fefop/edc-contract/search-suku/id/' + $( this ).val();
	...
	expensesType.on( 'change',
		...
		var urlHeader = null;
		if ( 'I' === $( this ).val() )
		    urlHeader = '/fefop/edc-contract/header-item/';
		else
		    urlHeader = '/fefop/edc-contract/header-employment/';
		...
		expenseContainer.find( '.portlet-body' ).load(
		    ...
		    {
			General.loading( false );
			Fefop.EDCContract.calcTotalExpenses();
		    }
		);
	    }
	);
	...
	$.ajax(
		...
		type: 'POST',
		dataType: 'json',
		url: General.getUrl( '/fefop/edc-contract/fetch-contract/' ),
		...
		success: function ( response )
		{
		   ...
		   Fefop.EDCContract.blockEditing();
	...
	var settings = {
		...
		Form.init();
		Fefop.EDCContract.initFormSearchEnterprise( modal );
	    }
	};
	...
	submit = function()
	{
		...
		data: data,
		dataType: 'text',
		url: General.getUrl( '/fefop/edc-contract/search-enterprise-forward' ),
		beforeSend: function()
		{
		    App.blockUI( form );
		},
		...
		success: function ( response )
		{
			...
			$( '#enterprise-list tbody a.action-ajax' ).click(
			    function()
			    {
				Fefop.EDCContract.setEnterprise( $( this ).data( 'id' ), modal );
			    }
			);
		    };
    ...
    setEnterprise: function( id, modal )
    {
	 $.ajax(
		...
		type: 'POST',
		dataType: 'json',
		url: General.getUrl( '/fefop/edc-contract/fetch-enterprise/' ),
		data: {id: id},
		beforeSend: function()
		{
		    General.loading( true );
		...
		success: function ( response )
		{
		   var setFinalEnterprise = function( checking )
		   {
			...
			Fefop.EDCContract.detailEnterprise( response.fk_id_fefpenterprise );
		   };
	...
	$( '#detail-enterprise' ).load(
	    General.getUrl( '/fefop/edc-contract/detail-enterprise/id/' + id ),
	    function()
	    {
	...
	var urlItem = 'I' === type ? 'add-detailed-expense' : 'add-detailed-employment';
	...
	$.ajax({
	    ...
	    url: General.getUrl( '/fefop/edc-contract/' + urlItem ),
	    beforeSend: function()
	    {
		General.loading( true );
	    },
    ...
    {
	var id = $( '#id_edc_contract' ).val();
	if ( General.empty( id ) )
	    return false;
	
	General.newWindow( General.getUrl( '/fefop/edc-contract/export/id/' + id ) );
    }
};
