...
Client.Client = {
	...
	configPersonCompetency: function(pane) {
		var form = pane.find('form');
		submit = function () {
			var obj = {
				callback: function (response) {
					if (response.status) {
						form.find('#id_percompetency').val(response.data.id_percompetency);
					}
				}
			};
			Form.submitAjax(form, obj);
	...
	configInterest: function(pane) {
		var form = pane.find('form');
		submit = function () {
			var obj = {
				callback: function (response) {
					if (response.status) {
						form.find('#id_perinterest').val(response.data.id_perinterest);
					}
				}
			};
			Form.submitAjax(form, obj);
