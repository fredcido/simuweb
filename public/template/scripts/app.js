...
var App = function () {
	...
	if ( $.cookie( 'page-sidebar' ) ) {
	    
	    var body = $('body');
            var sidebar = $('.page-sidebar');
	    
	    status = $.cookie( 'page-sidebar' );
	    if ( status == 'opened' ) {
                body.removeClass("page-sidebar-closed");
                if (body.hasClass('page-sidebar-fixed')) {
                    sidebar.css('width', '');
	...
	handleSidebarState();
		...
		$.cookie( 'page-sidebar', 'opened', {path: '/'} );
	...
	*/
		...
		e.stopPropagation();
		    ...
		    jQuery(this).trigger( 'portlet-close' );
                    el.slideUp(200);
                } else {
                    jQuery(this).removeClass("expand").addClass("collapse");
		    jQuery(this).trigger( 'portlet-open' );
                    el.slideDown(200);
	...
	jQuery('body').on('click', '.portlet-title', function (e) {
		e.preventDefault();
		e.stopPropagation();
                jQuery(this).find( '.tools .collapse, .tools .expand' ).trigger( 'click' );
		return false;
        });
    }
    ...
    var handleUniform = function () {
        ...
        var test = $("input[type=checkbox]:not(.toggle,.toggle-check), input[type=radio]:not(.toggle, .star)");
        if (test.size() > 0) {
            test.each(function () {
	...
	handleResponsive: function()
	{
        ...
        scrollTo: function (el, offeset) {
            pos = el ? el.offset().top - 40 : 0;
	    scroller = !el ? 'html,body' : ( el.closest( '.tab-content' ).length ? el.closest( '.tab-content' ) : 'html,body' );
        ...
        blockUI: function (el, centerY) {
            var el = jQuery(el); 
            el.block({
                    message: '<img src="' + BASE_URL + '/public/template/img/ajax-loading.gif" align="">',
	...
	getRandLayoutColorCode: function()
	{
	    var keys = [];
	    for( var k in layoutColorCodes ) keys.push(k);
	    
	    rand = Math.floor( Math.random() * keys.length );
	    return layoutColorCodes[keys[rand]];
	}
