...
class Client_Model_Mapper_Client extends App_Model_Abstract
{
     ...
     * @return int|bool
     */
    public function savePerCompetency()
    {
        $dbAdapter = Zend_Db_Table_Abstract::getDefaultAdapter();
        $dbAdapter->beginTransaction();
        try {

            $dbPerCompetency = App_Model_DbTable_Factory::get('PerCompetency');

            $id = parent::_simpleSave($dbPerCompetency, false);

            // Save the auditing
            $history = 'ATUALIZA KOMPETENSIA SIRA  HUSI KLIENTE: %s  - ATUALIZA KOMPETENSIA HUSI KLIENTE';
            $history = sprintf($history, $this->_data['fk_id_perdata']);
            $this->_sysAudit($history, Client_Form_ClientCompetency::ID);

            $dataHistory = array(
                'fk_id_perdata' => $this->_data['fk_id_perdata'],
                'fk_id_dec' => Zend_Auth::getInstance()->getIdentity()->fk_id_dec,
                'action' => 'ATUALIZA KOMPETENSIA',
                'description' => 'ATUALIZA KOMPETENSIA',
            );
    ...
    public function saveInterest()
    {
        ...
        try {
            ...
            $dbPerInterest = App_Model_DbTable_Factory::get('PerInterest');

            $id = parent::_simpleSave($dbPerInterest, false);

            // Save the auditing
            $history = 'ATUALIZA INTERESSE SIRA  HUSI KLIENTE: %s  - ATUALIZA INTERESSE HUSI KLIENTE';
            $history = sprintf($history, $this->_data['fk_id_perdata']);
            $this->_sysAudit($history, Client_Form_ClientCompetency::ID);
    ...
    public function getCompetency($id)
    {
        ...
        $select = $dbPerCompetency->select()
            ->from(array('pc' => $dbPerCompetency))
            ->setIntegrityCheck(false)
            ->where('pc.fk_id_perdata = ?', $id);

        return $dbPerCompetency->fetchRow($select);
    }
    ...
    public function getInterest($id)
    {
        ...
        $select = $dbPerInterest->select()
            ->from(array('pi' => $dbPerInterest))
            ->setIntegrityCheck(false)
            ->where('pi.fk_id_perdata = ?', $id)
            ->order('pi.id_perinterest DESC')
            ->limit(1);

        return $dbPerInterest->fetchRow($select);
    }
