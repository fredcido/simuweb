    ...
    */
    ...
    public function registerReport()
    {
		...
		$select = $select->joinLeft(
                        ...
                        'jv.id_jobvacancy = js.fk_id_jobvacancy',
                        array()
                    )->joinLeft(
						array( 's' => $dbScholarity ),
						's.id_perscholarity = js.fk_id_perscholarity',
						array( 'scholarity' => 'GROUP_CONCAT(DISTINCT s.scholarity)' )
                   )->joinLeft(
                        array( 'jt' => $dbJobTraining ),
                        'jv.id_jobvacancy = jt.fk_id_jobvacancy',
                        array()
                    )->joinLeft(
                        array( 't' => $dbScholarity ),
                        't.id_perscholarity = jt.fk_id_perscholarity',
                        array( 'training' => 'GROUP_CONCAT(DISTINCT t.scholarity)' )
                    )->joinLeft(
                        array( 'jl' => $dbJobLanguage ),
                        'jv.id_jobvacancy = jl.fk_id_jobvacancy',
                        array()
                    )->joinLeft(
                        array( 'l' => $dbLanguage ),
                        'l.id_perlanguage = jl.fk_id_perlanguage',
                        array( 'language' => 'GROUP_CONCAT(DISTINCT l.language)' )
                    );
