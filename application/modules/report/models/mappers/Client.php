...
class Report_Model_Mapper_Client extends App_Model_Mapper_Abstract
{
    ...
    public function visitPurposeReport()
    {
        ...
        $data = new ArrayObject(
            array(
                'total' => $this->visitPurpose($rows),
                'age' => $this->visitPurposeAge($rows),
                'school' => $this->visitPurposeSchoolLevel($rows),
                'graph' => $this->_graphs,
            )
        );
    ...
    public function visitPurpose($rows)
    {
        ...
        $this->_graphs[App_General_String::randomHash()] = App_Util_Chart::columnChart($column, 'Kliente husi Distritu');

        return $data;
    }

    public function visitPurposeAge($rows) {
        $groups = array(
            '15 - 24',
            '25 - 39',
            '40 - 54',
        ...
        $data = array(
            'groups' => $groups,
            'total' => array_fill_keys($groups, $totalModel),
            'percent' => array_fill_keys($groups, $totalModel),
            'rows' => array(),
        );

        $purposes = [];
        foreach ($rows as $row) {
            $ageGroup = $this->fitIntoAgeGroup($row);
            if (empty($ageGroup)) continue;

            $gender = $row['gender'] == 'FETO' ? 'woman' : 'man';

            $data['total'][$ageGroup][$gender]++;
            $data['total'][$ageGroup]['total']++;
            $data['total']['Total'][$gender]++;
            $data['total']['Total']['total']++;

            // $data['percent'][$ageGroup][$gender] = 100 * ($data['total'][$ageGroup][$gender] / $data['total'][$ageGroup]['total']);
            // $data['percent']['Total'][$gender] = 100 * ($data['total']['Total'][$gender] / $data['total'][$ageGroup]['total']);

            if (empty($purposes[$row['purpose']])) {
                $purposes[$row['purpose']] = array_fill_keys($groups, $totalModel);
            }

            $purposes[$row['purpose']][$ageGroup][$gender]++;
            $purposes[$row['purpose']]['Total'][$gender]++;
            $purposes[$row['purpose']]['Total']['total']++;
        }
        
        foreach ($groups as $group) {
            $data['percent'][$group]['man'] = 100 * ($data['total'][$group]['man'] / $data['total'][$group]['total']);
            $data['percent'][$group]['woman'] = 100 * ($data['total'][$group]['woman'] / $data['total'][$group]['total']);
            $data['percent'][$group]['total'] = 100;
        }
    ...
    public function visitPurposeSchoolLevel($rows) {
        $levels = array();
        foreach ($rows as $row) {
            $school = empty($row['max_level_scholarity']) ? 'LA IHA' : $row['max_level_scholarity'];
            $levels[$school] = $school;
        }
        ...
        $data = array(
            'levels' => $levels,
            'total' => array_fill_keys($levels, $totalModel),
            'percent' => array_fill_keys($levels, $totalModel),
            'rows' => array(),
        );
        ...
        foreach ($rows as $row) {
            ...
            $data['total'][$schoolLevel][$gender]++;
            $data['total'][$schoolLevel]['total']++;
            $data['total']['Total'][$gender]++;
            $data['total']['Total']['total']++;

            if (empty($purposes[$row['purpose']])) {
                $purposes[$row['purpose']] = array_fill_keys($levels, $totalModel);
            }

            $purposes[$row['purpose']][$schoolLevel][$gender]++;
            $purposes[$row['purpose']]['Total'][$gender]++;
            $purposes[$row['purpose']]['Total']['total']++;
        }

        foreach ($levels as $level) {
            $data['percent'][$level]['man'] = 100 * ($data['total'][$level]['man'] / $data['total'][$level]['total']);
            $data['percent'][$level]['woman'] = 100 * ($data['total'][$level]['woman'] / $data['total'][$level]['total']);
            $data['percent'][$level]['total'] = 100;
        }
    ...
    public function fitIntoAgeGroup($row) {
        $age = null;
        switch ($row['age']) {
            case $row['age'] >= 15 && $row['age'] <= 24:
                $age = '15 - 24';
